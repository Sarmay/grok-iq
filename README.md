...
- **两种执行模式**：完整对话用于看实际回复和指标；快速质量测试只请求几个字，用于诊断出口连通与质量，不检测思考缺失。
- **定时执行**：每个计划单独配置 Cron、时区、账号、方案、轮次和出口。
- **保存完整结果**：记录首 Token、生成耗时、输出 Token、TPS、分类、错误和响应正文。
- **风险标记**：按风险周期内全部出口样本的异常占比、最近连续异常、强信号和预期内容匹配情况计算账号状态；复测正常后会退出连续条件；校验标记匹配忽略大小写、全角半角和空白；次数、占比、权重、封顶和状态保底分都能热更新。
